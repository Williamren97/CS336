...
def run_multihead_self_attention(
    ...
    """
    attn = model.MultiheadSelfAttention(d_model=d_model, num_heads=num_heads, attn_pdrop=0.0, rope_theta=None)
    with torch.no_grad():
    ...
    return attn(in_features, token_positions=None)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    attn = model.MultiheadSelfAttention(
        d_model=d_model,
        num_heads=num_heads,
        attn_pdrop=0.0,
...
def run_rope(
    ...
    """
    return model.apply_rope(x=in_query_or_key, theta=theta, token_positions=token_positions, d_model=d_k)
...
def run_rmsnorm(
    ...
    """
    rms = model.RMSNorm(d_model=d_model, eps=eps)
    with torch.no_grad():
